...
INSET_BOX = (68.35, 38.25, 69.15, 38.75)   # Dushanbe and the Rudaki district around it
INSET_W = 280.0
...
def main():
    ...
    iproject, iscale, iheight = make_projection(*INSET_BOX, INSET_W, 0.0)

    def touches_inset(geom):
        x0, y0, x1, y1 = bbox_of(geom)
        return x0 < INSET_BOX[2] and x1 > INSET_BOX[0] and y0 < INSET_BOX[3] and y1 > INSET_BOX[1]
    ...
    for s in sites:
        ...
        x, y = project(lon, lat)
        ix, iy = iproject(lon, lat)
        placed.append({**{k: v for k, v in s.items() if k != "point"},
                       "lat": round(lat, 4), "lon": round(lon, 4), "anchor": note,
                       "x": round(x, 1), "y": round(y, 1), "ix": round(ix, 1), "iy": round(iy, 1)})

    site_districts = [s["adm2"] for s in sites if s.get("adm2")]

    def layers(proj, min_area, tol, only_inset=False):
        pick = (lambda g: touches_inset(g)) if only_inset else (lambda g: True)
        return {
    ...
    inset = layers(iproject, 3.0, 0.6, only_inset=True)
    data = {
        ...
        "inset": {"width": INSET_W, "height": round(iheight, 1), "bbox": list(INSET_BOX),
                  "ptPerDeg": round(iscale, 4), "kmPerPt": round(111.32 / iscale, 5), **inset},
        "sites": placed,
    }
    ...
    print(f"national {NAT_W:.0f}x{height:.0f}pt, {data['national']['kmPerPt']:.3f} km/pt; "
          f"inset {data['inset']['kmPerPt']:.3f} km/pt")
